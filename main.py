...
class TelegramAlertEngine:
    ...
    def send_buy_signal_alert(
        ...
        # Calculate dynamic monetary performance targets based on ₹1 Lakh seed capital
        initial_capital = 100000.0
        final_capital = initial_capital * (1.0 + (roi / 100.0))

        # Enhanced Telegram message layout with explicitly visible RSI factor
        message_payload = (
        ...
        try:
            ...
            if response.status_code == 200 and response_data.get("ok") is True:
                print(
                    f" ✓ Telegram alert delivered successfully to phone "
                    f"for {clean_name}!"
                )
            else:
...
class QlibPredictiveEngine:
    ...
    def generate_qlib_alpha_features(
        self,
        df: pd.DataFrame,
        ticker: str,
    ) -> pd.DataFrame:
        ...
        df["qlib_mean_reversion_20d"] = (
            df["close"].rolling(window=20).mean() / df["close"]
        )
        df["qlib_vol_normalized_return"] = (
            df["daily_return"]
    ...
    def compute_predictive_score(self, df: pd.DataFrame) -> float:
        ...
        return float(
            (latest_row["qlib_momentum_5d"] * 0.4)
            + (latest_row["qlib_mean_reversion_20d"] * 0.6)
        )
    ...
    def run_backtest_from_dataframe(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(len(df)):
            ...
            if (
                ...
                position_shares = (
                    cash / (1.0 + self.slippage_pct + self.stt_pct)
                ) / current_price
                cash = 0.0
                trade_count += 1
                ...
                cash = (
                    position_shares
                    * current_price
                    * (1.0 - self.slippage_pct - self.stt_pct)
                )
                position_shares = 0.0
                trade_count += 1
...
class YahooFinanceQuantPipeline:
    def __init__(
        ...
        self.default_exchange = self.config.get(
            "default_exchange",
            "NSE",
        )
        ...
        self.ticker_mappings = self._load_and_wrap_tickers(
            ticker_csv_path
        )
    ...
    def calculate_quant_metrics(
        ...
        df["rolling_volatility_ann"] = (
            df["daily_return"].rolling(window=21).std()
            * np.sqrt(252)
        )
...
if __name__ == "__main__":
    ...
    bot_token = (
        ...
        or pipeline.config.get("telegram_bot_token", "")
    )
    chat_id = (
        os.getenv("TELEGRAM_CHAT_ID")
        or pipeline.config.get("telegram_chat_id", "")
    )

    bot_token = str(bot_token).strip() if bot_token else ""
    chat_id = str(chat_id).strip() if chat_id else ""

    notifier = TelegramAlertEngine(
        token=bot_token,
        chat_id=chat_id,
    )
    ...
    for wrapped_ticker in pipeline.ticker_mappings:
        ...
        metrics_df = pipeline.calculate_quant_metrics(
            raw_df,
            wrapped_ticker,
        )
        ...
        qlib_df = qlib_engine.generate_qlib_alpha_features(
            metrics_df,
            wrapped_ticker,
        )
        ...
        ann_vol = (
            float(metrics_df["rolling_volatility_ann"].iloc[-1]) * 100
        )
        daily_var = (
            float(metrics_df["var_95_threshold"].iloc[-1]) * 100
        )

        # FIX: Extract the current live RSI value from the final index layer
        ...
        # =================================================================
        # ADVANCED ALGO CRITERIA EXTRACTION PLACEHOLDERS
        # =================================================================
        # To add future filters, extract latest values here. Examples:
        # current_volume = float(metrics_df["volume"].iloc[-1])
        # avg_volume_20d = float(
        #     metrics_df["volume"].rolling(window=20).mean().iloc[-1]
        # )
        # =================================================================

        # 4. GOLDEN RULE INTEGRATED FILTER SYSTEM (WITH ENFORCED LIVE RSI)
        # Enhanced validation matrix: Checked for Alpha, ROI, and tight
        ...
        if (
            ...
            # =============================================================
            # PLACEHOLDER: FUTURE FILTER STRATEGY BLOCKS
            # =============================================================
            # Example implementation for liquidity check:
            # if current_volume < (avg_volume_20d * 0.8):
            #     print(
            #         f" -> [{wrapped_ticker}] Bypassed status. "
            #         f"Failed Liquidity Filter."
            #     )
            #     continue
            # =============================================================

            print(
                f" -> [{wrapped_ticker}] Golden Rule & RSI Satisfied "
                f"(Alpha: +{alpha_score:.4f} | RSI: {current_rsi:.2f} | "
                f"ROI: +{strategy_roi:.2f}%). Dispatching alert..."
            )
            ...
            print(
                f" -> [{wrapped_ticker}] Bypassed status. Failed strict "
                f"filter threshold parameter rules "
                f"(Alpha/ROI/RSI boundary mismatch)."
            )

    print(
        "\n[Complete] Quant script loops finished successfully. "
        "Overwriting metrics..."
    )
    ...
    latest_scan_records = []
    processed_json_files = glob.glob(
        "data/processed/*_processed.csv"
    )

    for file_path in processed_json_files:
        ticker_raw = os.path.basename(file_path).replace(
            "_processed.csv",
            "",
        )
        clean_name = (
        ...
        alpha_path = (
            f"data/alpha_features/{ticker_raw}_qlib_features.csv"
        )
        ...
        if os.path.exists(alpha_path):
            try:
                ...
                latest_alpha = float(
                    df_a["qlib_momentum_5d"].iloc[-1]
                )
                latest_rsi = float(
                    df_a["rsi_14d"].iloc[-1]
                )

                # Checked alignment parity with the Telegram notification engine
                if (
                    latest_alpha > 0.01
                ...
                latest_scan_records.append(
                    {
                        ...
                        "ann_volatility_pct": float(
                            df_m["rolling_volatility_ann"].iloc[-1]
                        ) * 100,
                        "daily_var_95_pct": float(
                            df_m["var_95_threshold"].iloc[-1]
                        ) * 100,
